...
## v2.9.61-beta — Positions page: the roster comes first (owner backlog from 2026-08-15: "these two bits are taking up a lot of room especially once you have created a team already")

`renderEditTeam` toggles `body.editor-has-players` when the roster has any real name; CSS then collapses the three setup helpers: `#rosterImportBtn` → a quiet one-line ghost row (icon + "Auto-fill from photo", subtitle hidden), `#sampleSquadBtn` hidden, `#posTagsHint` → bare chip row (the explanatory prose sits in `.bulk-prose`, hidden). An empty new team keeps the full-size guidance. The first player row moves from ~y=560 to ~y=324 at 390px. **Also (owner: "the player detail pills better laid out" · "delete a player shouldn't be so obvious"):** each player is a `.pe-block` — name row `# · name · ★ · ×`, then a `.pe-grid` (72px label column) with **Positions** and **Side / Foot** rows aligned down the page; segmented pickers get a 32px hit height and aria-labels; ★ moves into the name row (`togglePlayerStar`) so it never wraps; the remove control is a quiet grey glyph (red only while pressed — §5.0: red for the moment of danger, not decoration), 44px hit area. Three lines per player instead of five. Then owner: "put each player's attributes into a unified card" — `.pe-block` is a card (`#0a1628`, hairline border, 12px radius), name row on top, hairline, attribute grid inside; inputs sit on the darker `--surface-input`. Owner-stamped as an exception to §5.0's box-per-row rule: a player is an entity, not a settings row. Then owner: "the footing is quite overbearing in the card" — Side/Foot 'Both' is the default and now sits selected-but-quiet (light neutral); only a deliberate L or R earns the solid accent fill, so a left-footer stands out and every other card is calm.

## v2.9.60-beta — One Plan page (owner: "I'm thinking the settings and sub plan actually merge — a lot of it is duplicated" · "make it clean in style like the current settings page")

Settings (`teamSettings`) and Sub Plan (`subOrderOv`) had converged: half length / sub interval / players-per-sub steppers on both, strategy on both (rows vs Auto/Custom toggle), formation tiles on both, the even-time hint twice, and a big clock on the plan page that the Game tab already owns. **Merged into one Plan page** — the §5.0 benchmark card (`renderTeamSettings`, unchanged: formation · strategy · timing · breaks-only, 600px measure) renders at the top of `subOrderOv`; "This game" below it: keeper, starting XI, the timeline with positions, projected minutes, Prev/LIVE/Next. Retired from the plan page: the Auto/Custom tabs (the strategy row IS the toggle), the players-per-sub stepper, the formation chips, the clock anchor. `openSubOrder()` works without a game (card + "set today's squad" note); with a game the plan appears. `saveTeamSettings()` re-plans the game below on any change (and still reaches the live engine per v2.9.47/58). **Nav back to four** — Game · Plan · Positions · Roster; `switchToView('settings')` routes to Plan. The `#teamSettings` screen shell stays in the DOM (hidden) for one version so nothing external breaks; the benchmark is now "the defaults card on Plan" (design.md §5.0 wording updated). **Also (owner: "on the game screen I should be able to see the full bench plan"):** the bench header carries a "Full plan ›" door to the Plan tab, and every later-wave pill is annotated from the plan — when she's due, for whom, at which position (`buildPlanTimeline` reused per render). Fixing that exposed a pre-existing mismatch: the plan page mapped "Auto" → Matched whenever `G.pairs` existed (always, after kick-off), so it simulated Matched while the game ran Equal time; `buildPlanTimeline`/`renderSubOrder` now follow `G.subStrategy` directly, and the plan's **first** upcoming sub is taken from `getNextSwap()` — the live engine's own answer — so the NEXT ON card, the bench annotations and the plan can never name different kids.

## v2.9.59-beta — The plan shows positions, and the bench opens it (owner: "on the sub plan page it has the full game swap plan but I need to know what positions they are assigned to as well" · "we should be able to click into the bench and view the full game plan")

The Plan page (`subOrderOv`) has its own whole-game simulator (`buildPlanTimeline`), separate from the auto-plan generator — it was minutes-only. It now runs the P8 rule for Equal time (`chooseOffByFit`), pairs incoming to slots by tag (`pairByFit`) for every strategy, applies the same `_reseatArr`, and stores `at` + `moves` on each event; `renderSubOrder` shows a position badge on every incoming kid and a "moves to" line per block. **Plan is back in the bottom nav** (Game · Plan · Positions · Settings · Roster) — owner reverses v2.9.11 ("add sub plan back in the bottom nav"): now that the page shows the whole-game plan *with positions* it is what a coach wants mid-match. A first cut put a "Full game plan ›" door on the bench header; superseded by the tab in the same version. **The 84px "≡ Sub Plan" header band on the Plan page is removed** (owner: "taking up too much room") — the brand bar is the header (§3.3) and the tab says where you are. Also `chooseOffByFit`'s candidate pool is now *tied with the most-played* (within half an interval) rather than tied with the c-th, with an on-longest tie-break — the Plan-page fairness invariants (spread ≤ 5m, no churn) had regressed under the first cut. Verified: Dragonflies (sim), Equal time — badges on every incoming kid, cover moves named. Also retired the Plan page's "EQUAL-TIME IDEAL" block (owner: "just repeating what is already selected") — it restated the steppers above it and the Settings tab's own even-time hint. **Owner direction for next:** Settings and Sub Plan have converged (timing, strategy, formation, even-time hint all duplicated) — merge into one Plan page: defaults on top (benchmark card), this game's plan below. To be mapped and stamped.

## v2.9.58-beta — Change the group size mid-game (owner: "changing the amount of subs grouped didn't work during a game")

Matched (`paired`) groups are built once at kick-off (`buildPairs`, field order, `cfg.sc` at the time). v2.9.47 made Settings-tab changes reach the live `cfg`, but the existing `G.pairs` kept their old size, so the next sub still swapped the old count (Equal time reads `cfg.sc` live, which is why it looked fine there). `saveTeamSettings` now detects a change to players-per-sub or strategy during a live game and calls `rebuildPairsLive()` — regroups the current outfield (keeper out) into groups of the new size, resets `pairIdx`, marks a Custom plan stale, syncs `G.subStrategy` (logged), saves, re-renders. Edge check: groups 2,2,2 → change to 1 → 1,1,1,1,1,1 and the next sub swaps one.
