...
|---|---|---|
| Email + password sign-in (v2.9.44) | `signInWithPassword` / `signUp` / `resetPasswordForEmail`; recovery link → set-new-password mode; drawer "Set / change password" for signed-in users. JWT stored, auto-refresh by Supabase JS. Magic links retired. Google / Apple OAuth buttons auto-show when each provider is enabled (reads `/auth/v1/settings`). | ✅ |
