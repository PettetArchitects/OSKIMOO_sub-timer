...
**Flap board (`flapBoard()` + `.flap-board`/`.flap-tile`)** — the **airport departure-board / split-flap** treatment. *(Not currently used on the game clock — that reverted to plain DSEG (§3.6) so the digits could be larger; the flap board remains available as a reusable component for any numeric/score display.)*
