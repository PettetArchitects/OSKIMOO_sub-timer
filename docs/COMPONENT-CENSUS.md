...
| 2 | **Sub-strategy chooser** | Team settings · game settings (`renderSubStratPicker`) · in-game modal (`#subStratOv`) | 🟡 v2.9.33: settings + s2 both `.set-group`/`.opt-row`, solid-green selection; the in-game modal keeps `--card` one-shot buttons (no selected state — acceptable) | Fold the modal in if it ever gains a selected state |
...
| 4 | **Secondary / outline / back** | `.btn.btn-o` + `.back-btn` | 🟡 v2.9.33: both classes NEUTRAL (red = danger only, ui-check-guarded; Delete team overrides inline); class migration to `.ui-btn--secondary` still open | `.ui-btn--secondary` + tone modifiers — migration step 4 |
...
| 10 | **Section labels** | ~43 uppercase eyebrow labels + 35 `<h2>` | 🟡 v2.9.33: `.sec-label` class exists; teamSettings + s2 + s1 use it (h2 bands gone there) | Sweep remaining screens onto `.sec-label`; card-internal eyebrows keep their 10px tier |
