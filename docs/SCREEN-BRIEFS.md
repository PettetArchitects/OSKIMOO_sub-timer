...
**Must show:** period clock + next-sub countdown (the pair), score, on-field XI with positions, bench with rest times, whose sub is next *(the next-on bench rows carry the loud treatment — v2.9.25, the sub manager's focus)*; **pre-kickoff and at breaks: the keeper door** (v2.9.16 — GK pill on the pitch naming the current keeper; opens the gloves picker; the first step of the setup/break ritual, on the game screen since the Plan page's picker lost its live-game doors in v2.9.11); **pre-kickoff only: the timing confirm strip** (v2.9.7) **+ the announce door** (v2.9.8); **at every sub: the relay card** (v2.9.9 — who on, for whom, at which position, in speaking order; tap or 12 s to dismiss; undo withdraws it; break rotations defer to the announce view) — and when a sub is due, **the swap as a relay card**: incoming name, outgoing name, POSITION, in speaking order, readable at arm's length (owner rehearsal, 2026-08-08: mid-play the coach's job is warning kids and giving clear position instructions — the screen scripts what the coach shouts).
