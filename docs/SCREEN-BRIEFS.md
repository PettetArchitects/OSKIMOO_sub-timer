...
**Secondary:** `confSub()`, `undoLastSub()`, `adjScore()`, `injurySub()` (long-press), `openGkPick()` (the keeper door, setup + breaks), `endPeriodNow()` (tap the period label — the half-time override, v2.9.19: the ref's whistle beats the clock; confirmed, no-op at a break or pre-kick), `cycleView()` / `toggleGameFormation()`, bench reorder, `clkAdj()`, game drawer
