...
**Shipped (v2.9.12):** the card tap IS the Play door — `selectTeam()` straight into squad select, `resumeActiveGame()` when that team's game is live. The PLAN door is the card's quiet clipboard button (plan tone) → `openTeamActions()`: Plan ahead / Past games / Edit / Share ("Play now" left the sheet — the card does it). `+ New Team` wears `ui-btn--secondary is-plan` — a planning job, no longer matchday red; first-run landing keeps its own primary CTA.
...
**Secondary:** `newTeam()`, `openTeamActions()` (the quiet PLAN door), `openTeamCodeEntry()` (shared-team code), `resumeActiveGame()` / `discardActiveGame()` (only when a game is in progress), `toggleGlobalMenu()`
...
**Mode:** PLAN
**Purpose:** Get the roster in and **set each player's default strengths** — the positions they play, side, foot — the data every Play-mode proposal (auto-fill, rotation, relay cards) draws from (owner, 2026-08-08: Plan mode teaches the engine; Play mode watches it propose). A midweek surface — density welcome here, and team creation belongs to midweek prominence, never matchday prominence. **Roster-only since v2.9.23** (owner directive, 2026-08-09): the Game Settings block (formation / sub strategy / timing) moved to its own Settings tab → `teamSettings`.
...
**Secondary:** `confSub()`, `undoLastSub()`, `adjScore()`, `injurySub()` (long-press), `openGkPick()` (the keeper door, setup + breaks), `endPeriodNow()` (the END HALF button next to PAUSE — the half-time override, v2.9.19: the ref's whistle beats the clock; confirmed, shown only mid-period, FULL TIME on the last period), `cycleView()` / `toggleGameFormation()`, bench reorder, game drawer *(the inline clock steppers left in v2.9.22 — timing is seasonal; the strip's Change link covers exceptions)*
**Must show:** period clock + next-sub countdown (the pair), score, on-field XI with positions, bench with rest times, whose sub is next *(the next-on wave is an aligned ↑ON | ↓OFF | AT table — v2.9.38, one benchmark card; loud treatment since v2.9.25, the sub manager's focus — and it escalates with the countdown: amber ≤30s, red ≤10s, bottom-drawer takeover on portrait phones ≤10s per design.md §5.8)*; **pre-kickoff and at breaks: the keeper door** (v2.9.16 — GK pill on the pitch naming the current keeper; opens the gloves picker; the first step of the setup/break ritual, on the game screen since the Plan page's picker lost its live-game doors in v2.9.11); **pre-kickoff: no chrome between the clocks and the pitch** (the v2.9.7 timing strip retired v2.9.38, the v2.9.8 announce link retired v2.9.40 — the pitch shows the full starting line-up; the announce view's door is the break ritual); **at every sub: the relay card** (v2.9.9 — who on, for whom, at which position, in speaking order; tap or 12 s to dismiss; undo withdraws it; break rotations defer to the announce view) — and when a sub is due, **the swap as a relay card**: incoming name, outgoing name, POSITION, in speaking order, readable at arm's length (owner rehearsal, 2026-08-08: mid-play the coach's job is warning kids and giving clear position instructions — the screen scripts what the coach shouts).
**Hierarchy (three states, three kings):** ambient — the countdown dominates (the screen answers "when"); sub-due — the relay card dominates and everything else recedes (the screen answers "who, where, say it"); break — keeper picker + the proposed next-period line-up as reviewable defaults dominate, ending in the announce view (owner, 2026-08-08: half-time = choose the new keeper → adjust formation if the game demands it → review/adjust default positions → tell everyone the new line-up — the setup ritual in miniature, with formation change as a first-class break action, not buried). The current screen's dominance-1.0 failure is trying to be all of these at once. **Score keeping is the standing exception** (owner, 2026-08-08): goals arrive at random moments — score entry stays one tap away in ALL states, never dominant, always undoable. **Attribution happens AT the goal** (owner, 2026-08-08, superseding the earlier "deferred to the summary" wording): the "Who scored?" prompt fires on the + tap, skippable — name it in the moment, because "later will be forgotten". The summary picker and edit-from-history (v2.9.15) are the safety net for skipped or mis-tagged goals, not the primary path.
**Boundaries:** time accrues only while running; undo restores exactly; field size preserved through every sub type; sub times match the Plan page (P3 ✓ block); backgrounding saves immediately (P4.5); **the Plan page is not reachable from the live game (v2.9.11)** — break adjustments happen on the field and end in the announce view.
...
**Secondary:** score confirm, `pickPotm()` (player-of-the-match chips — huddle-fast, one tap, no overlay; v2.9.13), opponent/location fields (deferred-friendly), `promptScorer()` on any of our goals in the game log ("name the scorer / change scorer", v2.9.18 — the scorer is nameable at the goal, at full time, or from history; never locked out), `showScr()` (skip/home)
**Must show:** final score front and centre for confirmation; the player-of-the-match chips (playing-time order — likely candidates float up); per-player minutes (true totals, including keeper time) **with the per-position breakdown under each name ("LB 20′ · FW 10′", v2.9.22 — sub-minute stints folded)**; the game log.
