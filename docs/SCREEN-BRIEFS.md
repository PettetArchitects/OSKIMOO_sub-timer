...
**Secondary:** `confSub()`, `undoLastSub()`, `adjScore()`, `injurySub()` (long-press), `openGkPick()` (the keeper door, setup + breaks), `cycleView()` / `toggleGameFormation()`, bench reorder, `clkAdj()`, game drawer
