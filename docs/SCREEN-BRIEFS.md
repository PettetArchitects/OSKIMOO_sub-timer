...
**Secondary:** `confSub()`, `undoLastSub()`, `adjScore()`, `injurySub()` (long-press), `openGkPick()` (the keeper door, setup + breaks), `endPeriodNow()` (the END HALF button next to PAUSE — the half-time override, v2.9.19: the ref's whistle beats the clock; confirmed, shown only mid-period, FULL TIME on the last period), `cycleView()` / `toggleGameFormation()`, bench reorder, `clkAdj()`, game drawer
