...
| # | Action | Expected result |
| --- | --- | --- |
| 1 | Reach a period break (HT, or **Q1/Q3 in quarter sports**) — naturally, or tap **END HALF next to PAUSE** when the ref's whistle beats the clock (v2.9.19: confirm → the period ends at the current clock; same break path; button only shows mid-period) | Break state shown; clock stopped ⚠️ quarter breaks under-tested |
...
| 2 | Add opponent / location, pick goal scorers — every one of our goals in the summary log carries **name the scorer / change scorer** (v2.9.18; same picker as at the goal) | Fields + scorer tags saved with the match; picking from the summary never wipes typed opponent/location text |
